...
void TestStrings() {
    ...
    auto t = Print(str_node);
    auto z = LoadJSON("\"Hello, \\\"everybody\\\"\""s);
//    if ( t == z){
...
void TestArray() {
    ...
    auto t = LoadJSON("[1, 1.23, \"Hello\"]"s).GetRoot();
    auto ishod = Print(arr_node);
    auto polych =Print(t);
     assert(t.IsArray());

     auto k = Print(arr_node);
     assert(arr.at(0).AsInt() == 1);
